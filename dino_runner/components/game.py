...
class Game:
    ...
    def update(self):
        ...
        self.player.update_projectile(self.game_speed, self.obstacle_manager)
        self.update_score()
    ...
    def draw(self):
        ...
        self.player.draw_projectile(self.screen)
        self.draw_score()
        self.draw_power_up_time()
        self.draw_roar_availability()
    ...
    def draw_power_up_time(self):
        if self.player.has_power_up:
            ...
            if time_to_show >= 0:
                self.print_text(
                    f"It's {self.player.type.capitalize()}in' time! {time_to_show} seconds remaining",
                    (0, 0, 0), (500, 40)
                )
            else:
    ...
    def draw_roar_availability(self):
        if self.player.roar_available:
            self.print_text("Roar attack available!", (0, 0, 0), (500, 65))
    ...
    def show_menu(self):
        ...
        if self.death_count == 0:
            ...
            self.screen.blit(RESET, (half_screen_width - 20, half_screen_height - 150))
            self.print_text(f"Your last score was: {self.last_score}", (0, 0, 0), (half_screen_width, half_screen_height - 25))
            self.print_text(f"You have died {self.death_count} times", (0, 0, 0), (half_screen_width, half_screen_height - 50))
            self.print_text(f"Your current best is: {self.current_highscore}", (0, 0, 0), (half_screen_width, half_screen_height))
